...
def reroll_character():
    """Reroll a character and return a new one"""
    try:
        data = request.json
        ...
        # Get a random character different from the current one
        ...
        character_data = {
            ...
            'character_role': new_character.character_role or 'neutral'
        }
        
        # Generate HTML for the new character card
        from flask import render_template_string
        character_html = render_template_string('''
        <div class="character-select-card" data-id="{{ character.id }}">
            <div class="card-body d-flex flex-column">
                <div class="character-image-container">
                    <img src="{{ character.image_url }}" class="card-img-top character-image" alt="{{ character.name }}">
                    <div class="selection-indicator position-absolute top-0 end-0 m-2">
                        <i class="fas fa-check-circle text-white"></i>
                    </div>
                </div>
                <h5 class="card-title mt-2">{{ character.name }}</h5>
                <div class="character-traits small text-muted mb-2">
                    {% if character.character_traits %}
                        {% for trait in character.character_traits[:3] %}
                            <span class="badge bg-secondary me-1">{{ trait }}</span>
                        {% endfor %}
                    {% endif %}
                </div>
                <div class="d-flex justify-content-between mt-auto">
                    <button class="btn btn-sm select-character-btn" data-character-id="{{ character.id }}">
                        <i class="fas fa-user-check"></i> Select
                    </button>
                    <button class="btn btn-sm btn-outline-secondary reroll-btn">
                        <i class="fas fa-dice"></i> Reroll
                    </button>
                </div>
                <input type="checkbox" name="selected_images[]" id="character{{ character.id }}" 
                       value="{{ character.id }}" class="d-none character-checkbox">
            </div>
        </div>
