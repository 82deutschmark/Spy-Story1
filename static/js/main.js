...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Character highlighting in storyboard page
    function initializeCharacterHighlighting() {
        // Only execute on storyboard page
        if (!document.querySelector('.storyboard-body')) return;
        ...
        const characterNames = Array.from(characterPortraits).map(portrait => {
            return {
                name: portrait.getAttribute('data-character-name'),
                element: portrait
            };
        });
        ...
        // Process the story content to highlight character names
        characterNames.forEach(character => {
            // Skip if no valid character name
            if (!character.name) return;
            
            // Get the display name (change dashes back to spaces)
            const displayName = character.name.replace(/-/g, ' ');
            const nameRegex = new RegExp(`\\b${displayName}\\b`, 'gi');
            
            // First check if the character name exists in the text
            if (!nameRegex.test(storyContent.innerHTML)) return;
            
            // Reset regex
            nameRegex.lastIndex = 0;
            
            // Replace the character name with a span for highlighting
            storyContent.innerHTML = storyContent.innerHTML.replace(
                nameRegex, 
                match => `<span class="character-highlight" data-character="${character.name}">${match}</span>`
            );
        });
        
        // Add event listeners to all highlighted characters
        const highlightedCharacters = document.querySelectorAll('.character-highlight');
        highlightedCharacters.forEach(highlight => {
            const characterName = highlight.getAttribute('data-character');
            const characterPortrait = document.querySelector(`.character-portrait-mini[data-character-name="${characterName}"]`);
            
            if (!characterPortrait) return;
            
            const portraitImg = characterPortrait.querySelector('img');
            const nameText = characterPortrait.querySelector('.character-mini-name');
            
            if (!portraitImg || !nameText) return;
            
            // Create tooltip element
            const tooltip = document.createElement('div');
            tooltip.className = 'character-tooltip';
            tooltip.innerHTML = `
                <img src="${portraitImg.src}" alt="${nameText.textContent}">
                <div class="tooltip-character-name">${nameText.textContent}</div>
            `;
            ...
            highlight.addEventListener('mouseenter', function() {
                ...
                tooltip.style.left = (highlightRect.width / 2 - tooltip.offsetWidth / 2) + 'px';
            });
    ...
    // Execute character highlighting when DOM is loaded
    document.addEventListener('DOMContentLoaded', function() {
        initializeCharacterHighlighting();
    });
